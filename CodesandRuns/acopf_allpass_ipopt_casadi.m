function [results, success] = acopf_allpass_ipopt_casadi(case_name, m, a_params, ipopt_overrides, dctype, dcdc, loading_condition_per)

% AC-OPF with rotated ALL-PASS kernel (CasADi + IPOPT, DC pre-rotation).
% Usage:
%   [res, ok] = acopf_allpass_ipopt_casadi('case118',  0);        % no RATE_A reduction
%   [res, ok] = acopf_allpass_ipopt_casadi('case118', 10);        % 10%% reduction on 20%% of lines
%   [res, ok] = acopf_allpass_ipopt_casadi('case118', 10, 0.5);
%   [res, ok] = acopf_allpass_ipopt_casadi('case118', 10, [0.3 0.2], struct('tol',1e-7));
%
% Requires: MATPOWER on path (loadcase, makeYbus, ext2int, int2ext, runpf, mpoption),
%           CasADi on path (import casadi.*).
%
% ADDED INPUT:
...
if nargin < 6, dcdc = 0; end
if nargin < 7 || isempty(loading_condition_per), loading_condition_per = 100; end
alpha = loading_condition_per/100;

assert(exist('loadcase','file')==2, 'MATPOWER not on path.');
assert(exist('runpf','file')==2,    'MATPOWER runpf not on path.');

try
    import casadi.*
catch
    error('CasADi not on MATLAB path. addpath(''.../casadi-<ver>-<platform>-matlab<ver>...'') first.');
end

% ---------- Load case & internal ordering ----------
...
% ---------- Ybus & edge list for PF balance ----------
[Ybus,~,~] = makeYbus(mpc);
absY = abs(Ybus); phiY = angle(Ybus);
[bi, bj] = find(absY ~= 0);          % include diagonal (internal indexing)
E = numel(bi);
...
% ---------- Auto a_params from angle band (if empty) ----------
if isempty(a_params)
    if any(has_l | has_u)
...
% ---------- Bus power balance via rotated ALL-PASS ----------
resE = (th(bi) - th(bj) - phi_e) - deltaE0;  % residual vs baseline (SX)
[ReR, ImR] = allpass_cascade_SX(resE, a_params);     % |r|=1
...
[ReF, ImF] = allpass_cascade_SX(rf, a_params);
[ReT, ImT] = allpass_cascade_SX(rt, a_params);
...
if any(has_l)
    ...
    ineq = [ineq; expr]; lb_ineq = [lb_ineq; zeros(nnz(has_l),1)]; ub_ineq = [ub_ineq; inf(nnz(has_l),1)];
end
if any(has_u)
    expr = angmax(has_u) - (th(fbus_all(has_u)) - th(tbus_all(has_u)));
    ineq = [ineq; expr]; lb_ineq = [lb_ineq; zeros(nnz(has_u),1)]; ub_ineq = [ub_ineq; inf(nnz(has_u),1)];
end
...
Pg_MW   = Pg_pu*baseMVA; 
Qg_Mvar = Qg_pu*baseMVA;
obj     = full(sol.f);

% ---------- Diagnostics ----------
% recompute balances with ALL-PASS at solution:
resE_val = (Va(bi) - Va(bj) - phi_e) - deltaE0;
[ReR_val, ImR_val] = allpass_cascade_num(resE_val, a_params);
ReE_val = c0E .* ReR_val - s0E .* ImR_val;
ImE_val = s0E .* ReR_val + c0E .* ImR_val;
Pf_bus_val = A * (Vm(bi).*Vm(bj).*absYe .* ReE_val);
Qf_bus_val = A * (Vm(bi).*Vm(bj).*absYe .* ImE_val);
...
% Use physical kernel argument: deltaE = theta_i - theta_j - phi_e.
% Direction convention: edge is from bus bj(e) -> bus bi(e).
deltaE_all_rad = Va(bi) - Va(bj) - phi_e;

% Only keep off-diagonal Ybus entries
is_offdiag = (bi ~= bj);
...
% Initialize PF/QF/PT/QT to NaN for all branches (incl out-of-service)
...
fprintf('Solver: CasADi + IPOPT | hess=limited-memory | linear_solver=mumps\n');
fprintf('Status: %s\n', rs);
...
% ================== helpers ==================
function [ReR, ImR] = allpass_cascade_SX(delta_vec, a_vec)
% CasADi SX version of r(δ) = Π_k (1 + j a_k δ) / (1 - j a_k δ), returned as (Re, Im).
% Uses real-only arithmetic:
% (1+jx)/(1-jx) = [(1-x^2) + j(2x)] / (1+x^2)
ReR = delta_vec*0 + 1;   % ones(size(delta_vec))
ImR = delta_vec*0;       % zeros
for kk = 1:numel(a_vec)
    x  = a_vec(kk) .* delta_vec;
    nr = 1 - x.^2;
    ni = 2 .* x;
    dr = 1 + x.^2;
...
function [ReR, ImR] = allpass_cascade_num(delta_vec, a_vec)
% Numeric (double) version for diagnostics after solve.
